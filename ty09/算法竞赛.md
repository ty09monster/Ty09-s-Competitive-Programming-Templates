...
对于长度为n的数组，分成根号n块，每块大小根号n，每个块维护区间信息，那么可以做到O(1)单点修改、O(根号)区间修改、O(根号)区间查询。

设块大小B=根号n。对于第i个数，它属于第i/B块
...
以下讨论利用欧拉序把树化为序列的树上莫队

本算法支持对路径的信息询问

用DFS维护树的欧拉序（长度变成了2n），st数组为节点第一次出现的时间，ed为节点第二次出现的时间。

假设查询路径u,v，且 `st[u]<=st[v]` 。
令 w = LCA(u,v)。
1.情况一：w=u（u 是 v 的祖先）
路径 u→v 对应欧拉序区间：`[ st[u],  st[v]]`
这个区间里，**出现奇数次的节点**恰好就是路径上的节点；出现偶数次的节点不在路径上。

2.情况二：w≠u
路径 u→v 对应欧拉序区间：`[en[u],st[v]]`
但注意：这个区间里**不包含** w=LCA(u,v)。  
所以还需要**额外加上** w 这个点。

剩下的内容用普通莫队维护

以下给定例题和模板代码
例题：给定有点权ai的无根树，每次询问给定u, v, l, r，询问在u,v这个路径上，定义f(x)为x这个点权的出现次数，那么需要求出 $\sum_{x=l}^r f(x)^2$  

思路：树上莫队加分块，树上莫队维护询问，分块维护f函数。时间复杂度 $O((n + q)\sqrt n)$ 
...
constexpr int UN = 1e5 + 10;      // 最大节点数/值域大小
constexpr int inf = 0X3F3F3F3F;
constexpr ll INF = 0X3F3F3F3F3F3F3F3F;

int n, q;                         // n个节点，q个询问
int B = 632;                      // 分块大小（用于莫队分块和值域分块）
int a[UN];                        // 每个节点的值

struct query {
    int ql, qr, ex, l, r, id;     // ql,qr:莫队区间左右端点; ex:额外点(LCA); l,r:值域查询范围; id:询问编号
} Q[UN];

int st[UN], ed[UN], id, eu[UN * 2]; // st:进入时间, ed:离开时间, id:欧拉序计数器, eu:欧拉序数组
vector<int> g[UN];                 // 邻接表存树

int top[UN], son[UN], siz[UN], dep[UN], fa[UN]; // 树链剖分：重链顶端、重儿子、子树大小、深度、父节点

// 第一次DFS：计算子树大小、重儿子、深度、父节点
...
void dfs1(int u, int t) {
    ...
    st[u] = ++id;                 // 记录进入时间
    eu[id] = u;                   // 欧拉序中记录节点
    if (son[u]) dfs1(son[u], t);  // 先递归重儿子，保持重链连续
    for (auto v : g[u]) {
        if (v == son[u] || v == fa[u]) continue; // 跳过重儿子和父节点
        dfs1(v, v);               // 轻儿子自己作为链头
    }
    ed[u] = ++id;                 // 记录离开时间
    eu[id] = u;                   // 欧拉序中再次记录节点
...
ll F[UN], f[UN], BF[UN];          // F[x] = f[x]*f[x]（当前值x出现次数的平方）, f[x]出现次数, BF:值域分块和

// 加入一个值为x的节点
void add(int x) {
    F[x] += 2 * f[x] + 1;         // (c+1)^2 - c^2 = 2c+1
    BF[x / B] += 2 * f[x] + 1;    // 更新值域分块和
    f[x]++;
}
...
int odd[UN];                      // 标记节点在欧拉序区间中出现次数的奇偶性，奇数次表示在路径上

// 根据奇偶性加入或删除节点
void check(int u) {
    if (odd[u])                   // 如果奇数次，说明在路径上，加入
        add(a[u]);
    else                          // 偶数次，不在路径上，删除
        del(a[u]);
}
...
void solve() {
    ...
    for (int i = 1; i <= q; i++) {
        int u, v, l, r;
        cin >> u >> v >> l >> r;
        if (st[u] > st[v])        // 保证st[u] <= st[v]
            swap(u, v);
        int w = lca(u, v);
        if (u == w) {             // 如果u是v的祖先，路径对应[st[u], st[v]]，无额外点
            Q[i] = {st[u], st[v], 0, l, r, i};
        } else {                  // 否则对应[ed[u], st[v]]，额外加上LCA
            Q[i] = {ed[u], st[v], w, l, r, i};
        }
    }

    // 莫队排序：按左端点所在块排序，同一块内右端点降序（奇偶优化）
    ...
    int L = 1, R = 0;             // 莫队当前维护的欧拉序区间[L, R]
    for (int i = 1; i <= q; i++) {
        auto [ql, qr, ex, l, r, id] = Q[i]; // 取出询问信息
        // 移动左指针（收缩）
        while (L < ql) {
            int u = eu[L++];
            odd[u] ^= 1;          // 翻转奇偶性
...
struct FastIO {
    ...
    char in_buf[SZ], *p = in_buf, *e = in_buf;
    char out_buf[SZ], *op = out_buf;
    ...
    char getc() {
        if (p == e)
            e = in_buf + fread(p = in_buf, 1, SZ, stdin);
        return p == e ? EOF : *p++;
    }
    ...
    void putc(char ch) {
        if (op - out_buf == SZ)
            fwrite(out_buf, 1, SZ, stdout), op = out_buf;
        *op++ = ch;
    }
